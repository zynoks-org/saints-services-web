...
export function WhyChooseUsTabs() {
  ...
  const tabContent: Record<string, TabData> = {
    ...
    professionalism: {
      ...
      title: 'Uncompromising Standards & SIA-Certified Officers',
      paragraphs: [
  ...
  return (
    <section className="py-16 sm:py-24 bg-[#070d1e] text-white border-b border-slate-800 relative overflow-hidden">
      ...
      {/* Ambient Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#f59e0b]/10 rounded-full blur-[160px] pointer-events-none z-0" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* HEADER */}
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-[#f59e0b] text-[11px] font-mono font-bold uppercase tracking-widest mb-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 bg-slate-900/90 border border-slate-800 p-1.5 rounded-2xl mb-8 sm:mb-12 shadow-xl backdrop-blur-md">
          {Object.values(tabContent).map((tab) => (
            <button
              ...
              className={`py-3.5 px-3 text-xs font-mono font-bold tracking-wider transition-all rounded-xl cursor-pointer text-center uppercase flex items-center justify-center gap-2 ${
                activeTab === tab.id
                  ? 'bg-[#f59e0b] text-[#0b1329] shadow-md font-black'
                  : 'text-slate-400 hover:text-white hover:bg-slate-800/60'
              }`}
            >
              <Activity className={`w-3.5 h-3.5 ${activeTab === tab.id ? 'text-[#0b1329]' : 'text-slate-500'}`} />
              <span>{tab.label}</span>
            </button>
          ))}
        </div>
        ...
        <div className="relative px-2">
          ...
          <div className="bg-[#0b1329] border border-slate-800 rounded-2xl sm:rounded-3xl p-6 sm:p-10 shadow-2xl relative z-10 overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
              
              {/* LEFT: TEXT CONTENT */}
              <div className="lg:col-span-7 space-y-4">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded bg-slate-900 border border-slate-800 text-[#f59e0b] text-[10px] font-mono font-bold tracking-widest uppercase">
                ...
                {current.paragraphs.map((paragraph, index) => (
                  <p key={index} className="text-slate-300 text-xs sm:text-sm font-medium leading-relaxed">
                    {paragraph}
                  </p>
                ))}
              ...
              <div className="lg:col-span-5 relative h-64 sm:h-80 lg:h-[380px] rounded-2xl overflow-hidden border border-slate-800 shadow-2xl bg-[#070d1e]">
                <Image 
                ...
                <div className="absolute bottom-4 left-4 right-4 p-3 bg-[#0b1329]/90 border border-slate-800 rounded-xl backdrop-blur-md flex items-center gap-3">
                  <ShieldCheck className="w-5 h-5 text-[#f59e0b] shrink-0" />
                  <span className="text-[11px] font-mono text-slate-300 font-bold uppercase tracking-wider">Verified Security Protocol</span>
                </div>
              </div>
